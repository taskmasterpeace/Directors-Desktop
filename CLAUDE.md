...
- **Vision inputs**: `treatment` (story sentences distributed across non-chorus sections in order), `artistName` (performance shots name them, waist-up framing), `directorStyle` (5 personas distilled from Palette's fingerprints — `server_utils/director_styles.py`, `GET /api/director/styles`). **Storyboard mode**: phases analyzing → `storyboarding` (keyframe image per shot via dp- models = Palette points, artist refs attached) → `awaiting_approval` (optional gate: `POST /api/director/storyboard/approve {runId, regenerate:[idx]}`) → generating (keyframe seeds each shot: i2v on Seedance, image conditioning on local A2V).

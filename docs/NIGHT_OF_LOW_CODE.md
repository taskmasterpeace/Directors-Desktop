...
| 9 | **"Open folder" in the Gallery header** — reveals the newest render in Explorer via the existing `showItemInFolder` IPC. The literal answer to "where do my generations actually live," one click from the surface that shows them. | ~18 | (this commit) |

...
| 8 | **Failed renders now show IN the queue strip** — red row with the actual error, a Retry (fresh seed, reuses the duplicate path), and dismiss. Before this a dead job just vanished; the strip only rendered running/queued/complete, which is exactly the "I don't know if it's still going" hole. | ~40 | (this commit) |

...
| 10 | **Drag-highlight blues → amber** on the gen surfaces: Playground FrameSlot + Gen Space's two image drop zones flashed LTX blue mid-drag. (Checked first whether FrameSlot even had drag-drop — it does, plus paste, done right.) Observed but deferred: a full app de-blue is ~75KB of hits (ExportModal, VideoEditor, ConfirmDialog…) — that's a themed sweep for a real work session, not a low-code night. | 3 | (this commit) |
